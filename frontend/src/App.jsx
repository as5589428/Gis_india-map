...
function App() {
  ...
  const [activeLayers, setActiveLayers] = useState(['india_boundary']);
      ...
      <main className="flex-1 relative ml-20">
        <header className={`absolute top-0 left-0 right-0 h-16 backdrop-blur-md border-b z-[1001] flex items-center px-8 shadow-sm pointer-events-auto transition-colors duration-300 ${isDark ? 'bg-gray-900/80 border-gray-800' : 'bg-white/80 border-gray-200'}`}>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-xl font-black tracking-tighter text-gray-900 italic">INDIA<span className="text-blue-600">-GIS</span></h1>
          </div>
          <div className="ml-auto flex items-center space-x-4">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50 px-2 py-1 rounded border border-gray-100">v1.2 Enterprise</span>
          </div>
        </header>
        ...
        <div className="absolute bottom-6 left-6 px-4 py-2 bg-white/80 backdrop-blur-sm rounded-lg text-[10px] font-mono text-gray-500 z-[1000] border border-gray-200">
